...
// Best-effort removal of a voice from the ElevenLabs account. Used to clean
// up PVC voices left behind by the retired PRO feature.
...
export async function generateClonedSpeech(text, voiceId, langCode = null, { v3 = false } = {}) {
  const apiKey = getElevenLabsApiKey();
  if (!apiKey) {
    throw new Error('ElevenLabs API key not configured');
  }
  ...
  const modelId = v3 ? ELEVEN_MODEL_V3 : resolveSpeechModelId(langCode);
  ...
  // sooner. v3 path: full 128 kbps, since quality is the whole point.
  const outputFormat = isV3 ? 'mp3_44100_128' : 'mp3_44100_64';
